...
public class HomeController {
    ...
    public String getRestaurants(HttpServletRequest request, Model model) {
        ...
        if(request.getParameter("ime").equals("") && request.getParameter("naselba").equals("")){
            ...
            restaurants = restaurantService.findAllByNameAndSuburb(request.getParameter("ime"),request.getParameter("naselba"));
        }
    ...
    public String getNearest(HttpServletRequest request,Model model){
        ...
        for (int i=0;i<restaurants.size();i++){
            distances.add(Math.sqrt(Math.pow(lon-restaurants.get(i).getLon(),2)+ Math.pow(lat-restaurants.get(i).getLat(),2)));
        }
